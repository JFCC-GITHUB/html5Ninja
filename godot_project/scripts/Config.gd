...
const CANVAS_WIDTH: float = 800.0
const CANVAS_HEIGHT: float = 450.0
const HIGH_SCORE_KEY: String = "ninja_game_high_score"

const PLAYER = {
	"INITIAL_HP": 3,
	"INVINCIBILITY_DURATION_MS": 1500,
	"SPEED": 4.5 * 60.0, # converted to pixels per second
	"WIDTH": 48.0,
	"HEIGHT": 64.0,
	"MOVE_BOUNDS_MARGIN": 80.0,
...
const SHURIKEN = {
	"SPEED": 9.0 * 60.0, # converted to pixels per second
	"BASE_SIZE": 20.0,
	"MAX_COUNT": 12,
}
...
const DIFFICULTIES = {
	"GOD": {
		...
		"stats": "初始血量: HP x10 | 滿級能力 (LV.10) | 敵人速度: 40% | 分數加成: x5.0",
	...
	"VERY_EASY": {
		...
		"stats": "初始血量: HP x5 | 敵人速度: 60% | 敵人生命: 70% | 分數加成: x0.8",
	...
	"EASY": {
		...
		"stats": "初始血量: HP x4 | 敵人速度: 80% | 敵人生命: 85% | 分數加成: x1.0",
	...
	"NORMAL": {
		...
		"stats": "初始血量: HP x3 | 敵人速度: 100% | 敵人生命: 100% | 分數加成: x1.5",
	...
	"HARD": {
		...
		"stats": "初始血量: HP x2 | 敵人速度: 125% | 敵人生命: 130% | 分數加成: x2.0",
	...
	"NIGHTMARE": {
		...
		"stats": "初始血量: HP x1 | 敵人速度: 150% | 敵人生命: 170% | 分數加成: x0.6",
		"enemySpeedMultiplier": 1.5, "enemyHpMultiplier": 1.7, "enemySpawnRateMultiplier": 0.6, "playerHp": 1, "startLevel": 1, "scoreMultiplier": 3.0
	}
}

const ENEMY_TYPES = {
	"NORMAL": { "id": "NORMAL", "name": "普通忍", "width": 42.0, "height": 58.0, "baseSpeed": 1.8 * 60.0, "hp": 1, "expValue": 20, "scoreValue": 100, "color": Color("#e74c3c"), "spawnWeight": 45 },
	"FAST": { "id": "FAST", "name": "疾風忍", "width": 36.0, "height": 50.0, "baseSpeed": 3.2 * 60.0, "hp": 1, "expValue": 35, "scoreValue": 200, "color": Color("#f1c40f"), "spawnWeight": 25 },
	"TANK": { "id": "TANK", "name": "重裝忍", "width": 58.0, "height": 74.0, "baseSpeed": 1.0 * 60.0, "hp": 3, "expValue": 60, "scoreValue": 350, "color": Color("#8e44ad"), "spawnWeight": 15 },
	"SHADOW": { "id": "SHADOW", "name": "影忍", "width": 40.0, "height": 54.0, "baseSpeed": 4.2 * 60.0, "hp": 2, "expValue": 80, "scoreValue": 500, "color": Color("#34495e"), "spawnWeight": 10, "reqLevel": 3 },
	"BOSS": { "id": "BOSS", "name": "鬼將", "width": 68.0, "height": 84.0, "baseSpeed": 1.2 * 60.0, "hp": 6, "expValue": 150, "scoreValue": 800, "color": Color("#2c3e50"), "spawnWeight": 5, "reqLevel": 5 }
}
...
const PARTICLES = { "HIT_COUNT": 12, "DEATH_COUNT": 25 }
